...
class TextClassifier:
    ...
    def preprocess_data(self, file_path):
        ...
        rows = []
        # Find all ticket blocks (each ticket and its description)
        ticket_pattern = re.findall(r"(Ticket\s*#[-\w]+[^\n\r]*)", content)
        ...
        for ticket_text in ticket_pattern:
            ...
            rows.append({"x_text": ticket_number, "y_label": y_label})

        df = pd.DataFrame(rows)
        print(f"✅ Found {len(df)} tickets (x_text now contains only ticket number)")
        return df
...
if __name__ == "__main__":
    combined_file_path = r"C:\Users\Windows\Downloads\ticket project\web_pages\combined_html_content.txt"
